...
def _ffmpeg_convert_audio(src: Path, dest: Path) -> Path:
    """Convert audio with ffmpeg when pydub is unavailable (e.g. Python 3.13+ without audioop-lts)."""
    dest.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(
...
def _tts_float(value: Any, default: float = 0.0) -> float:
    try:
...
def describe_tts_effects(
    *,
    ...
    noise_level = _tts_float(noise_level)
    background_tone_hz = _tts_float(background_tone_hz)
    background_tone_level = _tts_float(background_tone_level)
    pitch_semitones = _tts_float(pitch_semitones)
    speed_factor = _tts_float(speed_factor, 1.0)
    echo_delay_ms = _tts_float(echo_delay_ms)
    echo_decay = _tts_float(echo_decay)
    distortion = _tts_float(distortion)
    gain_db = _tts_float(gain_db)
    low_pass_hz = _tts_float(low_pass_hz)
    high_pass_hz = _tts_float(high_pass_hz)
    overlay_level = _tts_float(overlay_level)
    applied: list[str] = []
...
def create_tts_wav(
    ...
    *,
    ...
    if not _pydub_available():
        if effects:
            logging.warning(
                "pydub unavailable on this Python runtime; skipping TTS overlay/effects "
                "and converting speech to WAV via ffmpeg"
            )
        try:
            _ffmpeg_convert_audio(mp3_path, wav_path)
            mp3_path.unlink(missing_ok=True)
            return wav_path
        except Exception as exc:
            logging.warning("ffmpeg audio conversion failed (%s); returning MP3 TTS output", exc)
            return mp3_path
    ...
    try:
        ...
        if effects:
            logging.warning(
                "pydub unavailable (%s); skipping audio effects and using ffmpeg/plain output",
                exc,
            )
        try:
            _ffmpeg_convert_audio(mp3_path, wav_path)
